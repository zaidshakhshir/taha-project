...
<section class="section">
    ...
    <div class="section-body">
        ...
        <form class="container-fuild" action="{{ url('admin/tax/'.$tax->id) }}" method="post">
        @csrf
        @method('PUT')
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 mb-5">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is_invalide @enderror" placeholder="{{__('Tax Name')}}" value="{{ $tax->name }}" required="">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-5">
                            ...
                            <input type="number" name="tax" value="{{ $tax->tax }}" class="form-control @error('tax') is_invalide @enderror" placeholder="{{__('Tax')}}">
                            @error('tax')
                            <span class="custom_error" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="col-md-6 mb-5">
                            ...
                            <select name="type" class="form-control">
                                <option value="percentage" {{ $tax->type == 'percentage' ? 'selected' : '' }}>{{__('percentage')}}</option>
                                <option value="amount" {{ $tax->type == 'amount' ? 'selected' : '' }}>{{__('amount')}}</option>
                            </select>
                            @error('type')
                            <span class="custom_error" role="alert">
                                <strong>{{ $message }}</strong>
