@extends('layouts.app',['activePage' => 'selling_timeslot'])

@section('title','Vendor Selling Timeslots')

@section('content')

<section class="section">
    <div class="section-header">
        <h1>{{ $vendor->name }}&nbsp;{{ 'Selling timeslot'}}</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="{{ url('vendor/vendor_home') }}">{{__('Dashboard')}}</a></div>
            <div class="breadcrumb-item">{{__('Selling Time Slots')}}</div>
        </div>
    </div>
    <div class="section-body">
        <h2 class="section-title">{{__("Selling Time Slots")}}</h2>
        <p class="section-lead">{{__('Add and Edit Selling Time Slots')}}</p>
        <div class="card p-5">
            <div class="row">
                ...
                <div class="table-responsive">
                    <form action="{{ url('admin/update_selling_timeslot') }}" method="post">
                    @csrf
